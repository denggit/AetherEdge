"""Exchange-agnostic order ID validation and query-param resolution.

These functions are deliberately in the platform layer so both exchange
adapters (``src/platform/exchanges/*/client.py``) and order-management
modules can import them without creating a circular dependency.

Rules:
- Exchange-assigned order IDs (ordId, algoId, orderId, algoId) MUST be numeric.
- Fake / test / placeholder patterns MUST NOT be sent to exchange REST endpoints.
- Client-generated IDs (clOrdId, algoClOrdId, origClientOrderId, clientAlgoId)
  are valid as long as they are non-empty, non-sentinel strings.

Architecture constraint:
    ``src/platform`` must NOT import from ``src.order_management``.
"""
