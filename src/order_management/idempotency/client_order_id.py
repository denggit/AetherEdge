...
class DeterministicClientOrderIdFactory:
    ...
    The digest is derived from the stable ``intent_id`` (not wall-clock time),
    the signal action, the target exchange, and the planner item sequence.
    """
    ...
    def create(
        ...
        *,
        ...
        action_code = _ACTION_CODES.get(action.value, "xx")
        digest = hashlib.blake2b(
            f"{intent_id}|{action.value}|{exchange.value}|{sequence}".encode("utf-8"),
            digest_size=8,
        ).hexdigest().upper()
