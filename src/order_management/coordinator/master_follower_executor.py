...
from src.order_management.ports import MultiExchangeExecutorPort
...
class MasterFollowerExecutor:
    def __init__(
        ...
        *,
        ...
        executor: MultiExchangeExecutorPort,
    ) -> None:
        self.master_follower_policy = policy
        self._executor = executor

    async def execute(self, clients: Sequence[ExecutionClient], intent: OrderIntent, items: Sequence[PlannedExecution]) -> list[ExchangeOrderResult]:
        ...
        if self._bypasses_master_gating(intent):
            ...
            if purpose == "follower_close_after_master_close":
                ...
                results_nested = await asyncio.gather(
                    *(
                        self._executor.execute_for_client(
                            client,
                            intent,
                            items,
                ...
                results_nested = await asyncio.gather(*(self._executor.execute_for_client(client, intent, items) for client in clients))
        ...
        master_results = await self._executor.execute_for_client(
            master,
            intent,
            items,
