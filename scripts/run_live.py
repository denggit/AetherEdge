#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""AetherEdge formal live-runtime entrypoint."""
...
async def main() -> None:
    ...
    if runtime_mode is not RuntimeMode.LIVE_RUNTIME:
        raise LiveRuntimeError(
            "unsupported runtime mode for the formal live entrypoint: "
            f"{runtime_mode.value}; expected={RuntimeMode.LIVE_RUNTIME.value}"
        )
    is_direct_live = (
        PROJECT_ENV_CONFIG.get_bool("AETHER_LIVE_TRADING", False)
