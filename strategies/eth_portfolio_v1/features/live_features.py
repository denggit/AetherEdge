...
from strategies.eth_portfolio_v1.domain.models import ClosedKlineContext
from strategies.eth_portfolio_v1.features.indicators import adx, atr, ema, resample_ohlcv, rsi
...
def build_momentum_features(base_4h: pd.DataFrame, cfg: MomentumV3FeatureConfig) -> pd.DataFrame:
    ...
    out["close_gt_entry_high"] = out["close"] > out["entry_high"]
    out["close_lt_entry_low"] = out["close"] < out["entry_low"]
    out["vol_ok"] = vol_ok
    out["atr_ok"] = atr_ok
    out["adx_long_ok"] = out["adx"].between(cfg.min_adx_long, cfg.max_adx_long)
    out["adx_short_ok"] = out["adx"].between(cfg.min_adx_short, cfg.max_adx_short)
    out["short_enabled"] = bool(cfg.enable_short)
    out["ema20_gt_ema50"] = out["ema20"] > out["ema50"]
    out["ema20_lt_ema50"] = out["ema20"] < out["ema50"]
    out["close_gt_ema50"] = out["close"] > out["ema50"]
    out["close_lt_ema50"] = out["close"] < out["ema50"]
    out["close_gt_open"] = out["close"] > out["open"]
    out["close_lt_open"] = out["close"] < out["open"]
...
def build_bear_features(base_4h: pd.DataFrame, cfg: BearV3FeatureConfig) -> pd.DataFrame:
    ...
    out["four_h_bear"] = four_h_bear.fillna(False).astype(bool)
    out["breakdown"] = breakdown.fillna(False).astype(bool)
    out["permission_continuation"] = permission_continuation.fillna(False).astype(bool)
