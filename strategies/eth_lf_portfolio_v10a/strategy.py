...
class Strategy:
    ...
    def replace_range_speed_history(self, rf_bar_counts: Sequence[int]) -> int:
        ...
        if count >= self.config.entry_filters.range_speed_min_periods:
            logger.info(
                "V10A short-speed block available after range history refresh | complete_history=%s min_periods=%s",
                count,
                self.config.entry_filters.range_speed_min_periods,
            )
        else:
            logger.warning(
                "V10A short-speed block still unavailable after range history refresh | complete_history=%s min_periods=%s",
                count,
                self.config.entry_filters.range_speed_min_periods,
            )
        return count
