...
def test_client_order_id_is_deterministic_compact_and_exchange_specific():
    ...
    okx_id = factory.create(intent_id="test-intent", action=SignalAction.OPEN_LONG, exchange=ExchangeName.OKX, sequence=0)
    okx_id_again = factory.create(intent_id="test-intent", action=SignalAction.OPEN_LONG, exchange=ExchangeName.OKX, sequence=0)
    binance_id = factory.create(intent_id="test-intent", action=SignalAction.OPEN_LONG, exchange=ExchangeName.BINANCE, sequence=0)
    next_sequence_id = factory.create(intent_id="test-intent", action=SignalAction.OPEN_LONG, exchange=ExchangeName.OKX, sequence=1)

    assert okx_id == okx_id_again
    assert okx_id != binance_id
    assert okx_id != next_sequence_id
...
def test_repository_duplicate_guard_rejects_existing_intent(tmp_path):
    ...
    with pytest.raises(DuplicateIntentError):
        RepositoryDuplicateOrderGuard(repo).claim_or_raise(intent)
